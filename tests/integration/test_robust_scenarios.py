...
from tests.fixtures.test_fixtures import (
    basic_df, multi_date_df, messy_data_df, duplicate_data_df, 
    extreme_values_df, correlation_test_df, temporal_drift_df,
    basic_completeness_config, strict_completeness_config,
    basic_uniqueness_config, range_check_config, statistical_config,
    correlation_config, metadata_configs, edge_case_configs
)
...
class TestRobustIntegrationScenarios:
    ...
    def test_parallel_execution_with_complex_data(self, extreme_values_df):
        ...
        assert sequential_results["summary"]["total"] == parallel_results["summary"]["total"]
    ...
    def test_temporal_analysis_pipeline(self, temporal_drift_df):
        ...
        drifting_corr = [r for r in correlation_results if r["column"] == "drifting_value"][0]
        stable_corr = [r for r in correlation_results if r["column"] == "stable_value"][0]
        
        # Both checks should complete (pass, warning, or fail)
        assert drifting_corr["status"] in [CheckStatus.PASS, CheckStatus.WARNING, CheckStatus.FAIL]
        assert stable_corr["status"] in [CheckStatus.PASS, CheckStatus.WARNING, CheckStatus.FAIL]
    ...
    def test_error_resilience_and_recovery(self, basic_df, edge_case_configs):
        ...
        manager = CheckManager(
            df=basic_df,
            metadata=metadata,
            checks_config=problematic_config
        )
        ...
        error_results = [r for r in results["results"] if r.get("status") == CheckStatus.ERROR]
    ...
    def test_framework_initialization_edge_cases(self, basic_df, metadata_configs):
        ...
        config_dict = {
            ...
            "checks": {"completeness": {"value": {"thresholds": {"absolute_critical": 0.1}}}}
        }
    ...
    def test_mixed_data_types_handling(self):
        ...
        df = pd.DataFrame({
            ...
            "datetime_strings": ["2025-01-01", "invalid_date", "2025-01-03", None, "2025-01-05"]
        })
        ...
        for date in dates:
            for i in range(n_rows // n_dates):
                data.append({
                    ...
                    "category": np.random.choice(["A", "B", "C", None], p=[0.4, 0.3, 0.2, 0.1]),
