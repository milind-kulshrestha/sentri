...
from tests.fixtures.test_fixtures import (
    basic_df, multi_date_df, messy_data_df, extreme_values_df,
    statistical_config, edge_case_configs
)
...
class TestStatisticalCheck:
    ...
    def test_statistical_check_all_measures(self, sample_df):
        ...
        config = {
            "value": {
                "measures": ["mean", "median", "std", "min", "max", "count", "skew", "kurtosis"],
                "thresholds": {}
            }
        }
    ...
    def test_statistical_check_with_shared_fixtures(self, extreme_values_df, statistical_config):
    ...
    def test_statistical_check_with_infinite_values(self, messy_data_df):
        ...
        for _, result in results.iterrows():
            ...
            if not pd.isna(result["metric_value"]):
                assert np.isfinite(result["metric_value"]) or result["metric_value"] in [np.inf, -np.inf]
