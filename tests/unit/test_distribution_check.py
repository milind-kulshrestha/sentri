...
class TestDistributionCheck:
    ...
    def test_distribution_check_no_change(self, distribution_config):
        ...
        for period, date in enumerate([pd.Timestamp('2025-01-01'), pd.Timestamp('2025-01-02')]):
            for i in range(100):
                data.append({
                    'effective_date': date,
                    'entity_id': f'id_{i}',
    ...
    def test_distribution_check_insufficient_data(self, distribution_config):
        ...
        data = [
            {'effective_date': pd.Timestamp('2025-01-01'), 'entity_id': 'id_1', 'value': 1},
            {'effective_date': pd.Timestamp('2025-01-02'), 'entity_id': 'id_2', 'value': 2}
        ]
        ...
        assert not result.empty
        assert result.iloc[0]['status'] == CheckStatus.PASS
        assert 'Insufficient data for KS test' in result.iloc[0]['additional_metrics']['message']
